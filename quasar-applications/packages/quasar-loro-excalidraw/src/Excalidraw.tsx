...
function App({ client }: { client: QuasarClient }) {
  ...
  const { doc, docElements } = useMemo(() => {
    ...
    client.onMessage((e) => {
      console.log('received', e.length);
      const bytes = stringToUint8Array(e);
      try {
        doc.import(bytes);
      } catch (e) {
    ...
    doc.subscribe((e) => {
      ...
      if (e.by === "local") {
        ...
        const data =uint8ArrayToString(bytes);
        console.log('posting', data.length);
